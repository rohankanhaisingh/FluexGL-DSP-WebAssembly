...
(async function () {
    ...
    if (!fs.existsSync(wasmSourceDirectory) || !fs.existsSync(distSourceDirectory))
        return console.log("[ERROR]: ".red + `Could not build wasm because the source directory could not be located.`);
    ...
    async function internalThread() {
        return new Promise(function (resolve, reject) {
            cp.exec(`wasm-pack.exe build ${wasmSourceDirectory} --target no-modules --out-dir ${distSourceDirectory} --out-name fluexgl-dsp-wasm`, function(err, stdout, stderr) {
                stdout && console.log("[INFO]:".yellow + stdout);
                stderr && console.log("[INFO]:".yellow + stderr);
            }).on("close", function() {
                resolve();
            })
        });
    ...
    console.log("[SUCCES]: ".green + "Attempt to build was has succeed. See log above for building details.");
})();